...
from SprotifyMusic import app, Platform, LOGGER
from SprotifyMusic.utils import get_lang
...
async def scan_movie_folder(_, message: Message):
    ...
    if not query:
        return await message.reply_text("🎬 Enter the movie you want to seek.")
...
async def scan_anime_folder(_, message: Message):
    ...
    if not query:
        return await message.reply_text("🎬 Enter what you want to seek.")
...
async def send_results_page(message: Message, user_id: int):
    ...
    text = f"🎥 <b>Films Found: {len(files)}</b> - Page {page_index + 1}/{total_pages}</b>\n\n"
